...
namespace NodeAPI
{
	...
	{
		public CustomPreviousRowContent(string customNodeName, bool doesContain)
		{
		...
		{
			bool exists = previousNodes.Exists((NodeData x) => x.gridY == gridY - 1 && x is CustomSpecialNodeData && (x as CustomSpecialNodeData).Node != null && (x as CustomSpecialNodeData).Node.name == customNodeName);
